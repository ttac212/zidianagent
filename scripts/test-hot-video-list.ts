 ...
 * TikHub API 视频热榜测试脚本
 *
 * 测试 TikHub API 的视频热榜接口
 * 端点: /api/v1/douyin/billboard/fetch_hot_total_video_list (POST)
 *
 * 使用方法:
 * npx tsx scripts/test-hot-video-list.ts
 * npx tsx scripts/test-hot-video-list.ts --tag=628  # 美食垂类
 * npx tsx scripts/test-hot-video-list.ts --window=1 --size=20  # 按小时窗口，每页20条
 */
...
import type { HotVideoInfo, HotVideoListTag } from '@/lib/tikhub/types'
 ...
 * 测试获取视频热榜列表（全部垂类）
 */
async function testGetAllHotVideos(options: {
  dateWindow?: number
  pageSize?: number
}) {
  console.log('=== 测试获取视频热榜列表（全部垂类）===\n')
  ...
  try {
    ...
    console.log('正在获取视频热榜...\n')
    const response = await client.getHotVideoList({
      page: 1,
      page_size: pageSize,
    ...
    const outputPath = './hot-video-list-output.json'
    await fs.writeFile(
      outputPath,
      JSON.stringify(
        {
 ...
 * 测试获取特定垂类的视频热榜
 */
async function testGetCategoryHotVideos(options: {
  tagId: number
  tagName?: string
  dateWindow?: number
  pageSize?: number
  ...
  console.log(`=== 测试获取视频热榜列表（${tagName}垂类）===\n`)
  ...
  try {
    ...
    // 保存完整数据到文件
    const fs = await import('fs/promises')
    const outputPath = `./hot-video-list-${tagId}-output.json`
    await fs.writeFile(
      outputPath,
      JSON.stringify(
        {
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════╗')
  console.log('║     TikHub API - 视频热榜测试工具                ║')
  console.log('╚══════════════════════════════════════════════════╝\n')
  ...
  if (tagParam) {
    ...
    tests.push({
      name: `获取${tagName}垂类视频热榜`,
      fn: () =>
        testGetCategoryHotVideos({
          tagId,
          tagName,
          dateWindow,
          pageSize,
...
export { testGetAllHotVideos, testGetCategoryHotVideos }
