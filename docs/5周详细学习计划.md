# 智点AI平台 - 5周详细学习计划（210小时）

> **目标**：从基础到独立开发，系统掌握项目核心技术栈
> **学习强度**：每天6小时，共35天
> **适合人群**：有编程基础（Java/Python），前端基础较弱，需要系统学习

---

## 📅 学习计划总览

| 周次 | 主题 | 学习时长 | 核心目标 |
|------|------|----------|----------|
| Week 1 | 前端基础补强 | 42小时 | 掌握TypeScript + React + Next.js基础 |
| Week 2 | 项目核心流程 | 42小时 | 理解聊天系统完整链路 |
| Week 3 | 前端架构深入 | 42小时 | 掌握状态管理和缓存策略 |
| Week 4 | 高级主题 | 42小时 | SSE流式处理和并发控制 |
| Week 5 | 实战项目 | 42小时 | 独立开发新功能 |

---

# Week 1：前端基础补强（42小时）

## Day 1：TypeScript类型系统（6小时）

### 上午（3小时）：基础类型和接口

**学习内容：**
1. 基础类型（string, number, boolean, array, object）
2. 接口（interface）和类型别名（type）
3. 联合类型和交叉类型
4. 可选属性和只读属性

**推荐资源：**
- TypeScript官方文档：https://www.typescriptlang.org/docs/handbook/2/everyday-types.html
- 视频教程（1小时）：TypeScript入门 by Fireship

**实践任务：**
```typescript
// 创建文件：test-learning/day1-types.ts

// 任务1：定义用户类型
interface User {
  id: string
  email: string
  displayName?: string
  role: 'USER' | 'ADMIN' | 'GUEST'
  createdAt: Date
}

// 任务2：定义消息类型
interface Message {
  id: string
  content: string
  role: 'user' | 'assistant'
  createdAt: Date
}

// 任务3：定义对话类型（包含消息数组）
interface Conversation {
  id: string
  title: string
  userId: string
  messages: Message[]
  createdAt: Date
  updatedAt: Date
}

// 任务4：编写类型守卫函数
function isAdminUser(user: User): user is User & { role: 'ADMIN' } {
  return user.role === 'ADMIN'
}

// 测试
const testUser: User = {
  id: '1',
  email: 'test@test.com',
  role: 'ADMIN',
  createdAt: new Date()
}

if (isAdminUser(testUser)) {
  console.log('This is admin')
}
```

### 下午（3小时）：泛型和高级类型

**学习内容：**
1. 泛型基础（Generic Types）
2. 泛型约束（Generic Constraints）
3. 实用工具类型（Partial, Required, Pick, Omit）
4. 项目中的类型定义

**实践任务：**
```typescript
// 创建文件：test-learning/day1-generics.ts

// 任务1：编写泛型函数
function createArray<T>(length: number, value: T): T[] {
  return Array(length).fill(value)
}

const numbers = createArray<number>(3, 0) // [0, 0, 0]
const strings = createArray<string>(2, 'hello') // ['hello', 'hello']

// 任务2：编写泛型接口
interface ApiResponse<T> {
  data: T
  error: string | null
  success: boolean
}

// 使用示例
const userResponse: ApiResponse<User> = {
  data: { id: '1', email: 'test@test.com', role: 'USER', createdAt: new Date() },
  error: null,
  success: true
}

// 任务3：使用工具类型
type PartialUser = Partial<User> // 所有属性可选
type UserWithoutDates = Omit<User, 'createdAt'> // 排除某些属性
type UserEmailAndRole = Pick<User, 'email' | 'role'> // 只保留某些属性

// 任务4：阅读项目类型定义
// 打开并理解：types/chat.ts
```

**验证标准：**
- [ ] 能解释interface和type的区别
- [ ] 能编写简单的泛型函数
- [ ] 能使用Partial、Omit等工具类型
- [ ] 能读懂 `types/chat.ts` 中的类型定义

---

## Day 2：TypeScript在React中的应用（6小时）

### 上午（3小时）：React组件类型

**学习内容：**
1. 函数组件类型定义
2. Props类型定义
3. 事件处理器类型
4. Hooks类型（useState, useEffect）

**实践任务：**
```typescript
// 创建文件：test-learning/components/Button.tsx

import React from 'react'

// 任务1：定义Props类型
interface ButtonProps {
  children: React.ReactNode
  onClick?: () => void
  variant?: 'primary' | 'secondary' | 'danger'
  disabled?: boolean
  className?: string
}

// 任务2：编写类型化的函数组件
export function Button({
  children,
  onClick,
  variant = 'primary',
  disabled = false,
  className = ''
}: ButtonProps) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`btn btn-${variant} ${className}`}
    >
      {children}
    </button>
  )
}

// 任务3：编写带状态的组件
interface CounterProps {
  initialCount?: number
}

export function Counter({ initialCount = 0 }: CounterProps) {
  const [count, setCount] = React.useState<number>(initialCount)

  const handleIncrement = () => setCount(prev => prev + 1)
  const handleDecrement = () => setCount(prev => prev - 1)

  return (
    <div>
      <p>Count: {count}</p>
      <Button onClick={handleIncrement}>+</Button>
      <Button onClick={handleDecrement}>-</Button>
    </div>
  )
}
```

### 下午（3小时）：项目类型实践

**学习内容：**
1. 阅读 `types/chat.ts`
2. 理解项目中的核心类型
3. 学习类型扩展和模块声明

**实践任务：**
```bash
# 打开项目文件
code types/chat.ts
```

**详细阅读以下类型，并添加注释：**
```typescript
// types/chat.ts 关键类型解读

// 1. ChatMessage - 聊天消息
export interface ChatMessage {
  id: string                    // 唯一标识符
  role: 'user' | 'assistant'   // 消息角色
  content: string               // 消息内容
  status: 'pending' | 'streaming' | 'completed' | 'error'  // 消息状态
  reasoning?: string            // AI推理过程（可选）
  createdAt: Date              // 创建时间
  metadata?: Record<string, any> // 元数据（可选）
}

// 2. ChatState - 聊天状态（Reducer使用）
export interface ChatState {
  messages: ChatMessage[]       // 消息列表
  isLoading: boolean           // 是否正在加载
  error: string | null         // 错误信息
  // ... 其他字段
}

// 3. ChatAction - 聊天动作（Reducer使用）
export type ChatAction =
  | { type: 'SEND_USER_MESSAGE'; payload: { message: ChatMessage } }
  | { type: 'UPDATE_MESSAGE_STREAM'; payload: { id: string; content: string } }
  | { type: 'REMOVE_MESSAGE'; payload: { id: string } }
  // ... 其他动作
```

**任务清单：**
- [ ] 为每个类型添加中文注释
- [ ] 理解ChatMessage的status字段的4种状态
- [ ] 理解ChatAction的联合类型设计
- [ ] 画出ChatMessage的状态转换图

**验证标准：**
- [ ] 能解释ChatMessage中每个字段的作用
- [ ] 能说出消息的4种状态及其转换关系
- [ ] 能理解Reducer中Action的类型安全设计

---

## Day 3：React Hooks基础（6小时）

### 上午（3小时）：useState和useEffect

**学习内容：**
1. useState基础用法
2. useState的函数式更新
3. useEffect依赖数组
4. useEffect清理函数

**推荐资源：**
- React官方文档：https://react.dev/learn/state-a-components-memory
- 视频教程：React Hooks深入 by Web Dev Simplified

**实践任务：**
```typescript
// 创建文件：test-learning/components/TodoList.tsx

import React, { useState, useEffect } from 'react'

interface Todo {
  id: string
  text: string
  completed: boolean
}

export function TodoList() {
  // 任务1：使用useState管理状态
  const [todos, setTodos] = useState<Todo[]>([])
  const [inputValue, setInputValue] = useState('')

  // 任务2：使用useEffect从localStorage加载数据
  useEffect(() => {
    const saved = localStorage.getItem('todos')
    if (saved) {
      setTodos(JSON.parse(saved))
    }
  }, []) // 空依赖数组：只在组件挂载时执行

  // 任务3：使用useEffect保存数据到localStorage
  useEffect(() => {
    if (todos.length > 0) {
      localStorage.setItem('todos', JSON.stringify(todos))
    }
  }, [todos]) // 依赖todos：todos变化时执行

  // 添加待办事项
  const addTodo = () => {
    if (inputValue.trim()) {
      setTodos(prev => [
        ...prev,
        { id: Date.now().toString(), text: inputValue, completed: false }
      ])
      setInputValue('')
    }
  }

  // 切换完成状态
  const toggleTodo = (id: string) => {
    setTodos(prev =>
      prev.map(todo =>
        todo.id === id ? { ...todo, completed: !todo.completed } : todo
      )
    )
  }

  // 删除待办事项
  const deleteTodo = (id: string) => {
    setTodos(prev => prev.filter(todo => todo.id !== id))
  }

  return (
    <div>
      <input
        value={inputValue}
        onChange={e => setInputValue(e.target.value)}
        onKeyPress={e => e.key === 'Enter' && addTodo()}
      />
      <button onClick={addTodo}>Add</button>
      <ul>
        {todos.map(todo => (
          <li key={todo.id}>
            <input
              type="checkbox"
              checked={todo.completed}
              onChange={() => toggleTodo(todo.id)}
            />
            <span style={{ textDecoration: todo.completed ? 'line-through' : 'none' }}>
              {todo.text}
            </span>
            <button onClick={() => deleteTodo(todo.id)}>Delete</button>
          </li>
        ))}
      </ul>
    </div>
  )
}
```

### 下午（3小时）：useRef和useCallback

**学习内容：**
1. useRef访问DOM元素
2. useRef存储可变值
3. useCallback优化性能
4. useMemo vs useCallback

**实践任务：**
```typescript
// 创建文件：test-learning/components/ChatInput.tsx

import React, { useState, useRef, useCallback, useEffect } from 'react'

interface ChatInputProps {
  onSend: (message: string) => void
  disabled?: boolean
}

export function ChatInput({ onSend, disabled = false }: ChatInputProps) {
  const [input, setInput] = useState('')
  const textareaRef = useRef<HTMLTextAreaElement>(null)

  // 任务1：使用useRef聚焦输入框
  useEffect(() => {
    if (textareaRef.current && !disabled) {
      textareaRef.current.focus()
    }
  }, [disabled])

  // 任务2：使用useCallback优化函数
  const handleSend = useCallback(() => {
    if (input.trim() && !disabled) {
      onSend(input)
      setInput('')
      // 聚焦输入框
      textareaRef.current?.focus()
    }
  }, [input, disabled, onSend])

  // 任务3：处理快捷键（Ctrl+Enter发送）
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) {
      e.preventDefault()
      handleSend()
    }
  }, [handleSend])

  return (
    <div>
      <textarea
        ref={textareaRef}
        value={input}
        onChange={e => setInput(e.target.value)}
        onKeyDown={handleKeyDown}
        disabled={disabled}
        placeholder="输入消息... (Ctrl+Enter发送)"
        rows={3}
      />
      <button onClick={handleSend} disabled={disabled || !input.trim()}>
        发送
      </button>
    </div>
  )
}
```

**验证标准：**
- [ ] 能解释useState的函数式更新的好处
- [ ] 能解释useEffect依赖数组的作用
- [ ] 能使用useRef访问DOM和存储可变值
- [ ] 能解释useCallback的使用场景

---

## Day 4：React useReducer深入（6小时）

### 上午（3小时）：Reducer模式基础

**学习内容：**
1. Reducer概念（纯函数、不可变性）
2. Action和State设计
3. useReducer vs useState
4. 何时使用Reducer

**推荐资源：**
- React文档：https://react.dev/learn/extracting-state-logic-into-a-reducer
- Redux文档（理解概念）：https://redux.js.org/tutorials/fundamentals/part-3-state-actions-reducers

**实践任务：**
```typescript
// 创建文件：test-learning/reducers/todoReducer.ts

// 任务1：定义State类型
interface TodoState {
  todos: Array<{ id: string; text: string; completed: boolean }>
  filter: 'all' | 'active' | 'completed'
}

// 任务2：定义Action类型（联合类型）
type TodoAction =
  | { type: 'ADD_TODO'; payload: { text: string } }
  | { type: 'TOGGLE_TODO'; payload: { id: string } }
  | { type: 'DELETE_TODO'; payload: { id: string } }
  | { type: 'SET_FILTER'; payload: { filter: TodoState['filter'] } }
  | { type: 'CLEAR_COMPLETED' }

// 任务3：编写Reducer函数（纯函数）
export function todoReducer(state: TodoState, action: TodoAction): TodoState {
  switch (action.type) {
    case 'ADD_TODO':
      return {
        ...state,
        todos: [
          ...state.todos,
          {
            id: Date.now().toString(),
            text: action.payload.text,
            completed: false
          }
        ]
      }

    case 'TOGGLE_TODO':
      return {
        ...state,
        todos: state.todos.map(todo =>
          todo.id === action.payload.id
            ? { ...todo, completed: !todo.completed }
            : todo
        )
      }

    case 'DELETE_TODO':
      return {
        ...state,
        todos: state.todos.filter(todo => todo.id !== action.payload.id)
      }

    case 'SET_FILTER':
      return {
        ...state,
        filter: action.payload.filter
      }

    case 'CLEAR_COMPLETED':
      return {
        ...state,
        todos: state.todos.filter(todo => !todo.completed)
      }

    default:
      return state
  }
}

// 任务4：初始状态
export const initialTodoState: TodoState = {
  todos: [],
  filter: 'all'
}
```

### 下午（3小时）：在组件中使用Reducer

**实践任务：**
```typescript
// 创建文件：test-learning/components/TodoApp.tsx

import React, { useReducer, useMemo } from 'react'
import { todoReducer, initialTodoState } from '../reducers/todoReducer'

export function TodoApp() {
  // 任务1：使用useReducer
  const [state, dispatch] = useReducer(todoReducer, initialTodoState)
  const [input, setInput] = React.useState('')

  // 任务2：根据filter筛选todos（使用useMemo优化）
  const filteredTodos = useMemo(() => {
    switch (state.filter) {
      case 'active':
        return state.todos.filter(todo => !todo.completed)
      case 'completed':
        return state.todos.filter(todo => todo.completed)
      default:
        return state.todos
    }
  }, [state.todos, state.filter])

  // 任务3：dispatch不同类型的action
  const handleAdd = () => {
    if (input.trim()) {
      dispatch({ type: 'ADD_TODO', payload: { text: input } })
      setInput('')
    }
  }

  const handleToggle = (id: string) => {
    dispatch({ type: 'TOGGLE_TODO', payload: { id } })
  }

  const handleDelete = (id: string) => {
    dispatch({ type: 'DELETE_TODO', payload: { id } })
  }

  const handleFilterChange = (filter: 'all' | 'active' | 'completed') => {
    dispatch({ type: 'SET_FILTER', payload: { filter } })
  }

  const handleClearCompleted = () => {
    dispatch({ type: 'CLEAR_COMPLETED' })
  }

  return (
    <div>
      <h1>Todo App with Reducer</h1>

      {/* 输入区域 */}
      <div>
        <input
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyPress={e => e.key === 'Enter' && handleAdd()}
          placeholder="What needs to be done?"
        />
        <button onClick={handleAdd}>Add</button>
      </div>

      {/* 过滤器 */}
      <div>
        <button onClick={() => handleFilterChange('all')}>All</button>
        <button onClick={() => handleFilterChange('active')}>Active</button>
        <button onClick={() => handleFilterChange('completed')}>Completed</button>
        <button onClick={handleClearCompleted}>Clear Completed</button>
      </div>

      {/* Todo列表 */}
      <ul>
        {filteredTodos.map(todo => (
          <li key={todo.id}>
            <input
              type="checkbox"
              checked={todo.completed}
              onChange={() => handleToggle(todo.id)}
            />
            <span style={{ textDecoration: todo.completed ? 'line-through' : 'none' }}>
              {todo.text}
            </span>
            <button onClick={() => handleDelete(todo.id)}>×</button>
          </li>
        ))}
      </ul>

      {/* 统计 */}
      <div>
        {state.todos.filter(t => !t.completed).length} items left
      </div>
    </div>
  )
}
```

**验证标准：**
- [ ] 能解释Reducer的三个原则（纯函数、不可变、确定性）
- [ ] 能解释为什么要用联合类型定义Action
- [ ] 能解释何时用useReducer而不是useState
- [ ] 能独立编写简单的Reducer

---

## Day 5：Next.js App Router（6小时）

### 上午（3小时）：路由系统

**学习内容：**
1. App Router vs Pages Router
2. 文件系统路由
3. 动态路由 `[id]`
4. 路由组 `(group)`
5. 布局（Layout）和模板（Template）

**推荐资源：**
- Next.js官方文档：https://nextjs.org/docs/app/building-your-application/routing

**实践任务：**
```bash
# 创建Next.js测试项目
npx create-next-app@latest test-nextjs --typescript --app --no-src-dir

cd test-nextjs
```

**任务1：创建基础路由结构**
```
app/
├── page.tsx                    # 首页 /
├── about/
│   └── page.tsx               # 关于页 /about
├── blog/
│   ├── page.tsx               # 博客列表 /blog
│   └── [slug]/
│       └── page.tsx           # 博客详情 /blog/[slug]
└── dashboard/
    ├── layout.tsx             # Dashboard布局
    ├── page.tsx               # Dashboard首页 /dashboard
    └── settings/
        └── page.tsx           # 设置页 /dashboard/settings
```

**任务2：实现动态路由**
```typescript
// app/blog/[slug]/page.tsx

interface BlogPostProps {
  params: {
    slug: string
  }
}

export default function BlogPost({ params }: BlogPostProps) {
  return (
    <div>
      <h1>Blog Post: {params.slug}</h1>
      <p>This is the blog post with slug: {params.slug}</p>
    </div>
  )
}
```

**任务3：实现布局**
```typescript
// app/dashboard/layout.tsx

export default function DashboardLayout({
  children
}: {
  children: React.ReactNode
}) {
  return (
    <div>
      <nav>
        <a href="/dashboard">Dashboard</a>
        <a href="/dashboard/settings">Settings</a>
      </nav>
      <main>{children}</main>
    </div>
  )
}
```

### 下午（3小时）：服务端组件和客户端组件

**学习内容：**
1. Server Components vs Client Components
2. 'use client' 指令
3. 何时使用哪种组件
4. 数据获取模式

**实践任务：**
```typescript
// app/posts/page.tsx (服务端组件)

interface Post {
  id: number
  title: string
  body: string
}

// 服务端组件可以直接async
export default async function PostsPage() {
  // 直接在组件中获取数据
  const res = await fetch('https://jsonplaceholder.typicode.com/posts', {
    cache: 'no-store' // 禁用缓存
  })
  const posts: Post[] = await res.json()

  return (
    <div>
      <h1>Posts (Server Component)</h1>
      <ul>
        {posts.slice(0, 10).map(post => (
          <li key={post.id}>
            <h3>{post.title}</h3>
            <p>{post.body}</p>
          </li>
        ))}
      </ul>
    </div>
  )
}
```

```typescript
// app/counter/page.tsx

'use client' // 客户端组件标记

import { useState } from 'react'

// 客户端组件可以使用Hooks和事件处理
export default function CounterPage() {
  const [count, setCount] = useState(0)

  return (
    <div>
      <h1>Counter (Client Component)</h1>
      <p>Count: {count}</p>
      <button onClick={() => setCount(c => c + 1)}>Increment</button>
    </div>
  )
}
```

**验证标准：**
- [ ] 能创建基础路由和动态路由
- [ ] 能理解Layout的作用和使用场景
- [ ] 能区分Server Component和Client Component
- [ ] 能解释何时使用'use client'

---

## Day 6：Next.js API Routes（6小时）

### 上午（3小时）：API路由基础

**学习内容：**
1. API路由文件结构
2. HTTP方法处理（GET、POST、PATCH、DELETE）
3. Request和Response对象
4. 动态API路由

**实践任务：**
```typescript
// app/api/hello/route.ts

import { NextRequest, NextResponse } from 'next/server'

// GET /api/hello
export async function GET(request: NextRequest) {
  return NextResponse.json({
    message: 'Hello from API',
    timestamp: new Date().toISOString()
  })
}

// POST /api/hello
export async function POST(request: NextRequest) {
  const body = await request.json()

  return NextResponse.json({
    message: 'Data received',
    data: body
  })
}
```

**任务1：创建Todo API**
```typescript
// app/api/todos/route.ts

import { NextRequest, NextResponse } from 'next/server'

// 模拟数据库
let todos: Array<{ id: string; text: string; completed: boolean }> = []

// GET /api/todos - 获取所有todos
export async function GET() {
  return NextResponse.json({ todos })
}

// POST /api/todos - 创建新todo
export async function POST(request: NextRequest) {
  const { text } = await request.json()

  if (!text) {
    return NextResponse.json(
      { error: 'Text is required' },
      { status: 400 }
    )
  }

  const newTodo = {
    id: Date.now().toString(),
    text,
    completed: false
  }

  todos.push(newTodo)

  return NextResponse.json(
    { todo: newTodo },
    { status: 201 }
  )
}
```

**任务2：创建动态API路由**
```typescript
// app/api/todos/[id]/route.ts

import { NextRequest, NextResponse } from 'next/server'

// 注意：这里需要导入todos数组（实际项目中会用数据库）

// GET /api/todos/[id] - 获取单个todo
export async function GET(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const todo = todos.find(t => t.id === params.id)

  if (!todo) {
    return NextResponse.json(
      { error: 'Todo not found' },
      { status: 404 }
    )
  }

  return NextResponse.json({ todo })
}

// PATCH /api/todos/[id] - 更新todo
export async function PATCH(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const { completed } = await request.json()
  const index = todos.findIndex(t => t.id === params.id)

  if (index === -1) {
    return NextResponse.json(
      { error: 'Todo not found' },
      { status: 404 }
    )
  }

  todos[index] = { ...todos[index], completed }

  return NextResponse.json({ todo: todos[index] })
}

// DELETE /api/todos/[id] - 删除todo
export async function DELETE(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const index = todos.findIndex(t => t.id === params.id)

  if (index === -1) {
    return NextResponse.json(
      { error: 'Todo not found' },
      { status: 404 }
    )
  }

  todos.splice(index, 1)

  return NextResponse.json({ success: true })
}
```

### 下午（3小时）：集成前后端

**实践任务：创建完整的Todo应用**

```typescript
// app/todos-fullstack/page.tsx

'use client'

import { useState, useEffect } from 'react'

interface Todo {
  id: string
  text: string
  completed: boolean
}

export default function TodosFullstackPage() {
  const [todos, setTodos] = useState<Todo[]>([])
  const [input, setInput] = useState('')
  const [loading, setLoading] = useState(false)

  // 加载todos
  useEffect(() => {
    fetchTodos()
  }, [])

  const fetchTodos = async () => {
    const res = await fetch('/api/todos')
    const data = await res.json()
    setTodos(data.todos)
  }

  // 添加todo
  const handleAdd = async () => {
    if (!input.trim()) return

    setLoading(true)
    try {
      const res = await fetch('/api/todos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: input })
      })
      const data = await res.json()
      setTodos([...todos, data.todo])
      setInput('')
    } catch (error) {
      console.error('Failed to add todo:', error)
    } finally {
      setLoading(false)
    }
  }

  // 切换完成状态
  const handleToggle = async (id: string, completed: boolean) => {
    try {
      await fetch(`/api/todos/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ completed: !completed })
      })
      setTodos(todos.map(t =>
        t.id === id ? { ...t, completed: !completed } : t
      ))
    } catch (error) {
      console.error('Failed to toggle todo:', error)
    }
  }

  // 删除todo
  const handleDelete = async (id: string) => {
    try {
      await fetch(`/api/todos/${id}`, { method: 'DELETE' })
      setTodos(todos.filter(t => t.id !== id))
    } catch (error) {
      console.error('Failed to delete todo:', error)
    }
  }

  return (
    <div>
      <h1>Fullstack Todo App</h1>
      <div>
        <input
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyPress={e => e.key === 'Enter' && handleAdd()}
        />
        <button onClick={handleAdd} disabled={loading}>
          {loading ? 'Adding...' : 'Add'}
        </button>
      </div>
      <ul>
        {todos.map(todo => (
          <li key={todo.id}>
            <input
              type="checkbox"
              checked={todo.completed}
              onChange={() => handleToggle(todo.id, todo.completed)}
            />
            <span style={{ textDecoration: todo.completed ? 'line-through' : 'none' }}>
              {todo.text}
            </span>
            <button onClick={() => handleDelete(todo.id)}>Delete</button>
          </li>
        ))}
      </ul>
    </div>
  )
}
```

**验证标准：**
- [ ] 能创建基础API路由（GET、POST）
- [ ] 能创建动态API路由
- [ ] 能处理错误响应（400、404等）
- [ ] 能在前端调用API并更新UI

---

## Day 7：Next.js Middleware和认证（6小时）

### 上午（3小时）：Middleware基础

**学习内容：**
1. Middleware概念和作用
2. 请求拦截和重定向
3. 修改请求和响应头
4. 路径匹配配置

**实践任务：**
```typescript
// middleware.ts (项目根目录)

import { NextRequest, NextResponse } from 'next/server'

export function middleware(request: NextRequest) {
  // 任务1：记录请求日志
  console.log(`[${new Date().toISOString()}] ${request.method} ${request.url}`)

  // 任务2：检查认证（简单示例）
  const token = request.cookies.get('auth-token')

  // 如果访问/dashboard且未登录，重定向到/login
  if (request.nextUrl.pathname.startsWith('/dashboard') && !token) {
    return NextResponse.redirect(new URL('/login', request.url))
  }

  // 任务3：添加自定义响应头
  const response = NextResponse.next()
  response.headers.set('x-middleware-processed', 'true')

  return response
}

// 配置Middleware匹配路径
export const config = {
  matcher: [
    '/dashboard/:path*',  // 匹配/dashboard及其子路径
    '/api/:path*'         // 匹配所有API路由
  ]
}
```

### 下午（3小时）：简单认证实现

**实践任务：**
```typescript
// app/api/auth/login/route.ts

import { NextRequest, NextResponse } from 'next/server'

export async function POST(request: NextRequest) {
  const { email, password } = await request.json()

  // 简单验证（实际项目中要加密和查数据库）
  if (email === 'test@test.com' && password === 'password') {
    const response = NextResponse.json({
      success: true,
      user: { email, role: 'user' }
    })

    // 设置cookie（实际项目中用JWT）
    response.cookies.set('auth-token', 'simple-token', {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      maxAge: 60 * 60 * 24 // 1天
    })

    return response
  }

  return NextResponse.json(
    { error: 'Invalid credentials' },
    { status: 401 }
  )
}
```

```typescript
// app/api/auth/logout/route.ts

import { NextRequest, NextResponse } from 'next/server'

export async function POST(request: NextRequest) {
  const response = NextResponse.json({ success: true })

  // 清除cookie
  response.cookies.delete('auth-token')

  return response
}
```

```typescript
// app/login/page.tsx

'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'

export default function LoginPage() {
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const router = useRouter()

  const handleLogin = async () => {
    const res = await fetch('/api/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, password })
    })

    if (res.ok) {
      router.push('/dashboard')
    } else {
      alert('Login failed')
    }
  }

  return (
    <div>
      <h1>Login</h1>
      <input
        type="email"
        value={email}
        onChange={e => setEmail(e.target.value)}
        placeholder="Email"
      />
      <input
        type="password"
        value={password}
        onChange={e => setPassword(e.target.value)}
        placeholder="Password"
      />
      <button onClick={handleLogin}>Login</button>
      <p>Hint: test@test.com / password</p>
    </div>
  )
}
```

**验证标准：**
- [ ] 能创建Middleware并配置匹配路径
- [ ] 能实现简单的路由保护
- [ ] 能实现基础的登录/登出功能
- [ ] 能理解cookie和认证流程

**Week 1 总结：**
- [ ] 完成TypeScript基础学习
- [ ] 掌握React Hooks（useState、useEffect、useReducer）
- [ ] 理解Next.js路由和API系统
- [ ] 实现简单的全栈应用

---

# Week 2：项目核心流程（42小时）

## Day 8：Prisma和数据库（6小时）

### 上午（3小时）：Prisma基础

**学习内容：**
1. Prisma Schema语法
2. 模型定义和关系
3. Prisma Client API
4. 迁移和生成

**推荐资源：**
- Prisma官方文档：https://www.prisma.io/docs/getting-started

**实践任务：**
```bash
# 回到项目目录
cd D:\zdqidongxiangmu

# 打开Prisma Schema
code prisma/schema.prisma
```

**任务1：阅读并理解Schema**
```prisma
// prisma/schema.prisma

// 关键模型解读：

// 1. User模型
model User {
  id                  String         @id @default(cuid())
  email               String         @unique
  displayName         String?
  role                UserRole       @default(USER)

  // 配额管理字段
  monthlyTokenLimit   Int            @default(100000)
  currentMonthUsage   Int            @default(0)
  usageResetDate      DateTime       @default(now())

  // 关系字段
  conversations       Conversation[]  // 一个用户有多个对话
  messages            Message[]       // 冗余：一个用户有多个消息

  createdAt           DateTime       @default(now())
  updatedAt           DateTime       @updatedAt

  // 索引：优化查询性能
  @@index([email])
}

// 2. Conversation模型
model Conversation {
  id                  String         @id @default(cuid())
  title               String
  userId              String

  // 冗余字段：优化查询
  lastMessageAt       DateTime?      // 最后消息时间（避免JOIN）
  messageCount        Int            @default(0)
  totalTokens         Int            @default(0)

  // 关系
  user                User           @relation(fields: [userId], references: [id], onDelete: Cascade)
  messages            Message[]

  createdAt           DateTime       @default(now())
  updatedAt           DateTime       @updatedAt

  // 复合索引：优化对话列表查询
  @@index([userId, lastMessageAt(sort: Desc)])
}

// 3. Message模型
model Message {
  id                  String         @id @default(cuid())
  conversationId      String
  userId              String         // 冗余：避免JOIN查询配额
  role                MessageRole
  content             String

  // Token统计
  promptTokens        Int            @default(0)
  completionTokens    Int            @default(0)

  // 关系
  conversation        Conversation   @relation(fields: [conversationId], references: [id], onDelete: Cascade)
  user                User           @relation(fields: [userId], references: [id], onDelete: Cascade)

  createdAt           DateTime       @default(now())

  // 索引
  @@index([conversationId, createdAt])
  @@index([userId])
}
```

**任务2：理解冗余字段设计**
在文档中记录：
```markdown
## 冗余字段设计理由

### 1. Message.userId
- **冗余原因**：消息已经有conversationId，通过conversation可以找到userId
- **为什么冗余**：用户配额统计需要频繁查询用户的所有消息，避免JOIN提升性能
- **查询对比**：
  - 无冗余：SELECT * FROM Message JOIN Conversation ON ... WHERE Conversation.userId = ?
  - 有冗余：SELECT * FROM Message WHERE userId = ? (快10倍+)

### 2. Conversation.lastMessageAt
- **冗余原因**：可以通过messages关系查最新消息时间
- **为什么冗余**：对话列表按最后消息时间排序是高频操作
- **查询对比**：
  - 无冗余：需要JOIN messages表并GROUP BY
  - 有冗余：直接ORDER BY lastMessageAt (快100倍+)

### 3. Conversation.messageCount / totalTokens
- **冗余原因**：可以通过COUNT和SUM实时计算
- **为什么冗余**：避免每次显示对话列表都要聚合计算
```

### 下午（3小时）：Prisma Client实践

**实践任务：**
```typescript
// scripts/test-prisma-basics.ts

import { prisma } from '../lib/prisma'

async function main() {
  console.log('=== Prisma基础操作测试 ===\n')

  // 任务1：创建用户
  console.log('1. 创建测试用户...')
  const user = await prisma.user.create({
    data: {
      email: 'prisma-test@test.com',
      displayName: 'Prisma测试用户',
      role: 'USER',
      monthlyTokenLimit: 50000
    }
  })
  console.log('创建成功:', user)

  // 任务2：创建对话（带关系）
  console.log('\n2. 创建对话...')
  const conversation = await prisma.conversation.create({
    data: {
      title: '测试对话',
      userId: user.id,
      messages: {
        create: [
          {
            role: 'user',
            content: 'Hello',
            userId: user.id,
            promptTokens: 5
          },
          {
            role: 'assistant',
            content: 'Hi there!',
            userId: user.id,
            completionTokens: 10
          }
        ]
      }
    },
    include: {
      messages: true // 包含关联的消息
    }
  })
  console.log('创建成功:', conversation)

  // 任务3：查询用户的所有对话（带消息数）
  console.log('\n3. 查询用户对话...')
  const userConversations = await prisma.conversation.findMany({
    where: { userId: user.id },
    include: {
      _count: {
        select: { messages: true }
      }
    },
    orderBy: { lastMessageAt: 'desc' }
  })
  console.log('用户对话:', userConversations)

  // 任务4：聚合查询（统计token使用）
  console.log('\n4. 统计Token使用...')
  const tokenStats = await prisma.message.aggregate({
    where: { userId: user.id },
    _sum: {
      promptTokens: true,
      completionTokens: true
    },
    _count: true
  })
  console.log('Token统计:', tokenStats)

  // 任务5：事务操作（原子性）
  console.log('\n5. 事务操作...')
  await prisma.$transaction(async (tx) => {
    // 更新用户配额
    await tx.user.update({
      where: { id: user.id },
      data: { currentMonthUsage: { increment: 100 } }
    })

    // 更新对话统计
    await tx.conversation.update({
      where: { id: conversation.id },
      data: {
        messageCount: { increment: 1 },
        totalTokens: { increment: 100 }
      }
    })
  })
  console.log('事务执行成功')

  // 清理测试数据
  console.log('\n6. 清理测试数据...')
  await prisma.user.delete({ where: { id: user.id } })
  console.log('清理完成')
}

main()
  .catch(console.error)
  .finally(() => prisma.$disconnect())
```

**运行测试：**
```bash
npx tsx scripts/test-prisma-basics.ts
```

**验证标准：**
- [ ] 能解释User、Conversation、Message的关系
- [ ] 能理解3个冗余字段的设计理由
- [ ] 能使用Prisma Client进行CRUD操作
- [ ] 能理解事务的作用

---

## Day 9：数据库索引和性能（6小时）

### 上午（3小时）：索引策略

**学习内容：**
1. 数据库索引原理
2. 单列索引 vs 复合索引
3. 索引覆盖查询
4. 索引的权衡

**实践任务：**
```typescript
// scripts/test-index-performance.ts

import { prisma } from '../lib/prisma'

async function testIndexPerformance() {
  console.log('=== 索引性能测试 ===\n')

  // 准备测试数据：创建1个用户和100个对话
  console.log('准备测试数据...')
  const user = await prisma.user.create({
    data: {
      email: 'index-test@test.com',
      displayName: '索引测试用户'
    }
  })

  // 批量创建对话
  const conversations = []
  for (let i = 0; i < 100; i++) {
    conversations.push({
      title: `对话 ${i}`,
      userId: user.id,
      lastMessageAt: new Date(Date.now() - i * 1000 * 60 * 60) // 每个对话间隔1小时
    })
  }

  await prisma.conversation.createMany({ data: conversations })

  // 测试1：有索引的查询（使用复合索引）
  console.log('\n测试1：按userId和lastMessageAt排序查询（有索引）')
  const start1 = Date.now()
  const result1 = await prisma.conversation.findMany({
    where: { userId: user.id },
    orderBy: { lastMessageAt: 'desc' },
    take: 20
  })
  const time1 = Date.now() - start1
  console.log(`查询时间: ${time1}ms, 结果数: ${result1.length}`)
  console.log('使用的索引: [userId, lastMessageAt(sort: Desc)]')

  // 测试2：解释查询计划（SQLite）
  console.log('\n测试2：查看查询计划...')
  const explain = await prisma.$queryRaw`
    EXPLAIN QUERY PLAN
    SELECT * FROM Conversation
    WHERE userId = ${user.id}
    ORDER BY lastMessageAt DESC
    LIMIT 20
  `
  console.log('查询计划:', explain)

  // 清理
  await prisma.user.delete({ where: { id: user.id } })
  console.log('\n测试完成，数据已清理')
}

testIndexPerformance()
  .catch(console.error)
  .finally(() => prisma.$disconnect())
```

**理解索引配置：**
```prisma
model Conversation {
  // ...字段定义

  // 复合索引：优化对话列表查询
  // WHERE userId = ? ORDER BY lastMessageAt DESC
  @@index([userId, lastMessageAt(sort: Desc)])

  // 这个索引支持以下查询：
  // 1. WHERE userId = ?
  // 2. WHERE userId = ? ORDER BY lastMessageAt
  // 3. WHERE userId = ? ORDER BY lastMessageAt DESC
}

model Message {
  // ...字段定义

  // 索引1：优化获取对话消息
  @@index([conversationId, createdAt])

  // 索引2：优化用户配额统计
  @@index([userId])
}
```

### 下午（3小时）：Prisma Studio实战

**实践任务：**
```bash
# 启动Prisma Studio
pnpm db:studio
```

**任务清单：**
1. **浏览所有表**
   - [ ] 查看User表结构和数据
   - [ ] 查看Conversation表结构和数据
   - [ ] 查看Message表结构和数据
   - [ ] 理解表之间的关系

2. **手动创建数据**
   - [ ] 创建一个测试用户
   - [ ] 为该用户创建一个对话
   - [ ] 为该对话创建几条消息
   - [ ] 观察冗余字段（lastMessageAt等）是否正确

3. **测试级联删除**
   - [ ] 删除一个对话
   - [ ] 确认关联的消息也被删除
   - [ ] 理解onDelete: Cascade的作用

4. **验证索引效果**
   - [ ] 创建50+条对话
   - [ ] 在Studio中按lastMessageAt排序
   - [ ] 观察查询速度

**验证标准：**
- [ ] 能解释复合索引的工作原理
- [ ] 能解释索引覆盖查询
- [ ] 能使用Prisma Studio管理数据
- [ ] 能理解级联删除

---

## Day 10-11：认证系统深入（12小时）

### Day 10上午（3小时）：NextAuth.js深入

**学习内容：**
1. NextAuth.js架构
2. Provider概念
3. Session和JWT策略
4. Callbacks（jwt、session）

**推荐资源：**
- NextAuth.js文档：https://next-auth.js.org/getting-started/introduction

**实践任务：**
```bash
# 打开项目认证文件
code auth.ts
code auth/strategies/index.ts
code middleware.ts
```

**任务1：理解认证流程**
```typescript
// auth.ts 关键代码解读

import NextAuth from 'next-auth'
import Credentials from 'next-auth/providers/credentials'
import { selectAuthStrategy } from './auth/strategies'

export const { handlers, signIn, signOut, auth } = NextAuth({
  // 1. Provider配置
  providers: [
    Credentials({
      credentials: {
        email: { label: 'Email', type: 'email' },
        code: { label: 'Code', type: 'password' }
      },
      // 2. 认证逻辑
      async authorize(credentials) {
        const strategy = selectAuthStrategy()
        return await strategy.authenticate(credentials)
      }
    })
  ],

  // 3. Session策略：JWT（无需数据库）
  session: { strategy: 'jwt' },

  // 4. JWT回调：将用户信息注入token
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        // 登录时：将用户信息存入token
        token.userId = user.id
        token.role = user.role
        token.monthlyTokenLimit = user.monthlyTokenLimit
      }
      return token
    },

    // 5. Session回调：将token信息暴露给客户端
    async session({ session, token }) {
      if (token) {
        session.user.id = token.userId
        session.user.role = token.role
        session.user.monthlyTokenLimit = token.monthlyTokenLimit
      }
      return session
    }
  }
})
```

**画出认证流程图：**
```
用户登录
  ↓
1. 前端调用 signIn('credentials', { email, code })
  ↓
2. NextAuth调用 authorize(credentials)
  ↓
3. selectAuthStrategy() 选择策略
  ├─ 开发环境：developmentAuth
  │   ├─ 验证 DEV_LOGIN_CODE
  │   ├─ 查询或创建用户
  │   └─ 返回 User对象
  └─ 生产环境：productionAuth
      ├─ 验证 ADMIN_LOGIN_PASSWORD
      ├─ 仅查询已存在用户
      └─ 返回 User对象或null
  ↓
4. 如果返回User对象，触发 jwt callback
  ├─ 将userId、role等信息存入JWT token
  └─ 返回包含用户信息的token
  ↓
5. 触发 session callback
  ├─ 从token提取信息
  └─ 注入到session.user
  ↓
6. 前端获得session，登录成功
```

### Day 10下午（3小时）：双模式认证策略

**实践任务：**
```typescript
// auth/strategies/development.ts 解读

import { prisma } from '@/lib/prisma'

export const developmentAuth = {
  async authenticate(credentials: any) {
    const { email, code } = credentials

    // 验证开发环境认证码
    if (code !== process.env.DEV_LOGIN_CODE) {
      return null
    }

    // 查找或创建用户（自动注册）
    let user = await prisma.user.findUnique({
      where: { email }
    })

    if (!user) {
      // 开发环境：自动创建用户
      user = await prisma.user.create({
        data: {
          email,
          displayName: email.split('@')[0],
          role: 'USER',
          monthlyTokenLimit: 100000
        }
      })
      console.log('[Dev Auth] 自动创建用户:', email)
    }

    return user
  }
}
```

```typescript
// auth/strategies/production.ts 解读

import { prisma } from '@/lib/prisma'

export const productionAuth = {
  async authenticate(credentials: any) {
    const { email, code } = credentials

    // 验证生产环境密码
    if (code !== process.env.ADMIN_LOGIN_PASSWORD) {
      return null
    }

    // 只查询已存在的用户（不自动创建）
    const user = await prisma.user.findUnique({
      where: { email }
    })

    if (!user) {
      console.log('[Prod Auth] 用户不存在:', email)
      return null
    }

    return user
  }
}
```

**任务：写一个测试脚本**
```typescript
// scripts/test-auth-strategies.ts

import { selectAuthStrategy } from '../auth/strategies'

async function testAuthStrategies() {
  console.log('=== 认证策略测试 ===\n')

  // 测试当前环境策略
  const strategy = selectAuthStrategy()
  console.log('当前策略:', strategy)

  // 测试开发环境认证
  console.log('\n测试1：开发环境认证')
  console.log('环境变量 DEV_LOGIN_CODE:', process.env.DEV_LOGIN_CODE)

  const devResult = await strategy.authenticate({
    email: 'newuser@test.com',
    code: process.env.DEV_LOGIN_CODE
  })
  console.log('认证结果:', devResult ? '成功' : '失败')
  if (devResult) {
    console.log('用户信息:', {
      email: devResult.email,
      role: devResult.role,
      limit: devResult.monthlyTokenLimit
    })
  }

  // 测试错误认证码
  console.log('\n测试2：错误认证码')
  const failResult = await strategy.authenticate({
    email: 'test@test.com',
    code: 'wrong-code'
  })
  console.log('认证结果:', failResult ? '成功' : '失败（预期）')
}

testAuthStrategies()
  .catch(console.error)
  .finally(() => process.exit())
```

### Day 11上午（3小时）：Middleware路由保护

**实践任务：**
```typescript
// middleware.ts 详细解读

import { NextRequest, NextResponse } from 'next/server'
import { auth } from './auth'

// 公开路径：不需要认证
const publicPaths = [
  '/',
  '/login',
  '/api/auth',
  '/_next',
  '/favicon.ico'
]

export default async function middleware(req: NextRequest) {
  const { pathname } = req.nextUrl

  // 1. 检查是否是公开路径
  const isPublicPath = publicPaths.some(path =>
    pathname.startsWith(path)
  )

  if (isPublicPath) {
    return NextResponse.next()
  }

  // 2. 验证JWT token
  const session = await auth()

  // 3. 未登录：重定向到登录页
  if (!session?.user) {
    const loginUrl = new URL('/login', req.url)
    loginUrl.searchParams.set('callbackUrl', pathname) // 记住原始路径
    return NextResponse.redirect(loginUrl)
  }

  // 4. 已登录：注入userId到header（供API使用）
  const response = NextResponse.next()
  response.headers.set('x-user-id', session.user.id)
  response.headers.set('x-user-role', session.user.role)

  return response
}

// 配置：对所有路径生效，但publicPaths除外
export const config = {
  matcher: [
    '/((?!_next/static|_next/image|favicon.ico).*)'
  ]
}
```

**理解关键点：**
```typescript
// 为什么要注入header？
// 因为API路由中需要知道当前用户ID，但无法直接访问session

// app/api/chat/route.ts
export async function POST(req: NextRequest) {
  // 方式1：从header获取（Middleware注入）
  const userId = req.headers.get('x-user-id')

  // 方式2：重新验证token
  const session = await auth()
  const userId2 = session?.user?.id

  // 项目中使用方式2（更安全）
}
```

### Day 11下午（3小时）：完整认证流程实践

**实践任务：测试完整登录流程**

1. **创建测试用户**
```bash
npx tsx scripts/create-user.ts auth-test@test.com "认证测试" USER 50000
```

2. **测试登录流程**
```bash
# 启动开发服务器
pnpm dev

# 浏览器访问
http://localhost:3007/login
```

3. **观察认证流程**
- [ ] 打开浏览器开发者工具（F12）
- [ ] Network标签页
- [ ] 输入邮箱和认证码，点击登录
- [ ] 观察请求：
  - `POST /api/auth/signin` - 登录请求
  - 响应：设置cookie `authjs.session-token`
  - 重定向到首页或callbackUrl

4. **验证Session**
```typescript
// 在任意客户端组件中
'use client'

import { useSession } from 'next-auth/react'

export default function TestSession() {
  const { data: session, status } = useSession()

  if (status === 'loading') return <div>Loading...</div>
  if (status === 'unauthenticated') return <div>未登录</div>

  return (
    <div>
      <h2>当前用户</h2>
      <pre>{JSON.stringify(session.user, null, 2)}</pre>
    </div>
  )
}
```

5. **测试路由保护**
- [ ] 登出后访问 `/chat` - 应该重定向到 `/login`
- [ ] 登录后访问 `/chat` - 正常显示

**验证标准：**
- [ ] 能解释JWT和Session策略的区别
- [ ] 能解释双模式认证的设计理由
- [ ] 能解释Middleware的作用
- [ ] 能独立测试完整认证流程

---

## Day 12-14：聊天API完整链路（18小时）

### Day 12：配额管理系统（6小时）

**重要提示**：这是整个项目最核心的部分之一，务必理解透彻！

### 上午（3小时）：原子性操作原理

**学习内容：**
1. 竞态条件（Race Condition）
2. 数据库原子操作
3. 乐观锁 vs 悲观锁
4. SQL条件更新

**核心概念：为什么需要原子性？**
```typescript
// ❌ 错误示例：读-检查-写（Race Condition）
async function wrongReserveTokens(userId: string, tokens: number) {
  // 步骤1：读取当前使用量
  const user = await prisma.user.findUnique({ where: { id: userId } })

  // 步骤2：检查是否超限
  if (user.currentMonthUsage + tokens > user.monthlyTokenLimit) {
    throw new Error('配额不足')
  }

  // 步骤3：更新使用量
  await prisma.user.update({
    where: { id: userId },
    data: { currentMonthUsage: user.currentMonthUsage + tokens }
  })
}

// 问题：如果两个请求同时执行，可能都通过检查，导致超限
// 请求A: 读取usage=8000 → 检查8000+3000=11000 > 10000? 否 → 更新为11000
// 请求B: 读取usage=8000 → 检查8000+3000=11000 > 10000? 否 → 更新为11000
// 结果：usage变成14000，超过限制！
```

```typescript
// ✅ 正确示例：原子性条件更新
async function correctReserveTokens(userId: string, tokens: number) {
  const result = await prisma.user.updateMany({
    where: {
      id: userId,
      // 关键：在WHERE子句中检查条件
      currentMonthUsage: {
        lte: prisma.raw`monthlyTokenLimit - ${tokens}`
      }
    },
    data: {
      currentMonthUsage: { increment: tokens }
    }
  })

  // 如果count=0，说明更新失败（超限）
  if (result.count === 0) {
    throw new Error('配额不足')
  }
}

// 优点：数据库保证原子性，不会出现竞态条件
```

### 下午（3小时）：QuotaManager深入

**实践任务：**
```bash
# 打开配额管理器
code lib/security/quota-manager.ts
```

**详细阅读并添加注释：**
```typescript
// lib/security/quota-manager.ts 关键代码解读

export class QuotaManager {
  // 方法1：预留配额（原子性操作）
  async reserveTokens(userId: string, estimatedTokens: number): Promise<void> {
    // 步骤1：检查并重置月度配额（如果需要）
    await this.checkAndResetMonthlyUsage(userId)

    // 步骤2：原子性更新（使用Prisma原生SQL）
    const result = await prisma.$executeRaw`
      UPDATE User
      SET currentMonthUsage = currentMonthUsage + ${estimatedTokens}
      WHERE id = ${userId}
      AND currentMonthUsage + ${estimatedTokens} <= monthlyTokenLimit
    `

    // 步骤3：检查更新结果
    if (result === 0) {
      throw new QuotaExceededError('月度配额已用完')
    }

    console.log(`[Quota] 预留成功: userId=${userId}, tokens=${estimatedTokens}`)
  }

  // 方法2：提交实际使用量（事务）
  async commitTokens(
    userId: string,
    actualTokens: number,
    estimatedTokens: number,
    messageData: any
  ): Promise<void> {
    const adjustment = actualTokens - estimatedTokens

    // 使用事务确保一致性
    await prisma.$transaction(async (tx) => {
      // 步骤1：创建消息记录
      await tx.message.create({
        data: {
          ...messageData,
          promptTokens: messageData.role === 'user' ? actualTokens : 0,
          completionTokens: messageData.role === 'assistant' ? actualTokens : 0
        }
      })

      // 步骤2：调整用户配额（按差额）
      if (adjustment !== 0) {
        await tx.user.update({
          where: { id: userId },
          data: { currentMonthUsage: { increment: adjustment } }
        })
      }

      // 步骤3：更新对话统计
      await tx.conversation.update({
        where: { id: messageData.conversationId },
        data: {
          messageCount: { increment: 1 },
          totalTokens: { increment: actualTokens },
          lastMessageAt: new Date()
        }
      })
    })

    console.log(`[Quota] 提交成功: actual=${actualTokens}, adjustment=${adjustment}`)
  }

  // 方法3：释放配额（失败回滚）
  async releaseTokens(userId: string, tokens: number): Promise<void> {
    await prisma.user.update({
      where: { id: userId },
      data: { currentMonthUsage: { decrement: tokens } }
    })

    console.log(`[Quota] 释放成功: userId=${userId}, tokens=${tokens}`)
  }

  // 方法4：月度重置检查
  private async checkAndResetMonthlyUsage(userId: string): Promise<void> {
    const user = await prisma.user.findUnique({ where: { id: userId } })
    if (!user) return

    const now = new Date()
    const resetDate = new Date(user.usageResetDate)

    // 如果超过1个月，重置
    if (now.getTime() - resetDate.getTime() > 30 * 24 * 60 * 60 * 1000) {
      await prisma.user.update({
        where: { id: userId },
        data: {
          currentMonthUsage: 0,
          usageResetDate: now
        }
      })
      console.log(`[Quota] 月度重置: userId=${userId}`)
    }
  }
}

export const quotaManager = new QuotaManager()
```

**编写测试脚本：**
```typescript
// scripts/test-quota-manager.ts

import { quotaManager } from '../lib/security/quota-manager'
import { prisma } from '../lib/prisma'

async function testQuotaManager() {
  console.log('=== 配额管理器测试 ===\n')

  // 创建测试用户
  const user = await prisma.user.create({
    data: {
      email: 'quota-test@test.com',
      displayName: '配额测试',
      monthlyTokenLimit: 1000,
      currentMonthUsage: 0
    }
  })
  console.log('创建测试用户:', user.email)

  try {
    // 测试1：预留配额（成功）
    console.log('\n测试1：预留500 tokens（应该成功）')
    await quotaManager.reserveTokens(user.id, 500)
    const user1 = await prisma.user.findUnique({ where: { id: user.id } })
    console.log('当前使用量:', user1?.currentMonthUsage) // 应该是500

    // 测试2：预留配额（成功）
    console.log('\n测试2：再预留400 tokens（应该成功）')
    await quotaManager.reserveTokens(user.id, 400)
    const user2 = await prisma.user.findUnique({ where: { id: user.id } })
    console.log('当前使用量:', user2?.currentMonthUsage) // 应该是900

    // 测试3：预留配额（失败 - 超限）
    console.log('\n测试3：尝试预留200 tokens（应该失败）')
    try {
      await quotaManager.reserveTokens(user.id, 200)
      console.log('❌ 测试失败：应该抛出错误')
    } catch (error) {
      console.log('✅ 正确抛出错误:', (error as Error).message)
    }

    // 测试4：释放配额
    console.log('\n测试4：释放500 tokens')
    await quotaManager.releaseTokens(user.id, 500)
    const user4 = await prisma.user.findUnique({ where: { id: user.id } })
    console.log('当前使用量:', user4?.currentMonthUsage) // 应该是400

    console.log('\n✅ 所有测试通过')
  } finally {
    // 清理
    await prisma.user.delete({ where: { id: user.id } })
    console.log('\n测试数据已清理')
  }
}

testQuotaManager()
  .catch(console.error)
  .finally(() => prisma.$disconnect())
```

**运行测试：**
```bash
npx tsx scripts/test-quota-manager.ts
```

**验证标准：**
- [ ] 能解释竞态条件的问题
- [ ] 能解释原子性SQL更新的原理
- [ ] 能解释事务的作用
- [ ] 能解释预留-提交-回滚的三步模式

---

### Day 13：上下文裁剪和SSE基础（6小时）

### 上午（3小时）：上下文裁剪器

**实践任务：**
```bash
# 打开上下文裁剪器
code lib/chat/context-trimmer.ts
code lib/constants/message-limits.ts
```

**详细阅读：**
```typescript
// lib/constants/message-limits.ts

export const MESSAGE_LIMITS = {
  // 不同模型的上下文限制
  'claude-opus-4': {
    maxContextTokens: 180000,  // API调用限制
    displayTokens: 500000,     // 界面显示限制
    estimateRatio: 1.5         // 中文字符/token比率
  },
  'gpt-4': {
    maxContextTokens: 120000,
    displayTokens: 120000,
    estimateRatio: 2
  },
  // 默认配置
  default: {
    maxContextTokens: 100000,
    displayTokens: 100000,
    estimateRatio: 2
  }
}
```

```typescript
// lib/chat/context-trimmer.ts

import { ChatMessage } from '@/types/chat'
import { MESSAGE_LIMITS } from '@/lib/constants/message-limits'

// Token估算函数
function estimateTokens(text: string, ratio: number): number {
  // 简单估算：字符数 / 比率
  // 中文：1.5字符/token
  // 英文：4字符/token
  return Math.ceil(text.length / ratio)
}

// API调用时的裁剪（严格限制）
export function trimForChatAPI(
  messages: ChatMessage[],
  modelId: string
): ChatMessage[] {
  const config = MESSAGE_LIMITS[modelId] || MESSAGE_LIMITS.default
  const { maxContextTokens, estimateRatio } = config

  let totalTokens = 0
  const trimmedMessages: ChatMessage[] = []

  // 从最新消息开始向前遍历
  for (let i = messages.length - 1; i >= 0; i--) {
    const message = messages[i]
    const messageTokens = estimateTokens(message.content, estimateRatio)

    // 如果加上这条消息会超限，停止
    if (totalTokens + messageTokens > maxContextTokens) {
      break
    }

    totalTokens += messageTokens
    trimmedMessages.unshift(message) // 添加到开头
  }

  console.log(`[Trimmer] 裁剪: ${messages.length} → ${trimmedMessages.length} 消息`)
  console.log(`[Trimmer] 估算tokens: ${totalTokens} / ${maxContextTokens}`)

  return trimmedMessages
}

// 界面显示时的裁剪（宽松限制）
export function trimForDisplay(
  messages: ChatMessage[],
  modelId: string
): ChatMessage[] {
  // 类似逻辑，但使用displayTokens限制
  const config = MESSAGE_LIMITS[modelId] || MESSAGE_LIMITS.default
  // ... 省略实现
}
```

**编写测试脚本：**
```typescript
// scripts/test-context-trimmer.ts

import { trimForChatAPI } from '../lib/chat/context-trimmer'
import { ChatMessage } from '../types/chat'

function generateTestMessages(count: number): ChatMessage[] {
  const messages: ChatMessage[] = []
  for (let i = 0; i < count; i++) {
    messages.push({
      id: `msg-${i}`,
      role: i % 2 === 0 ? 'user' : 'assistant',
      content: '这是一条测试消息'.repeat(100), // 约1000字符
      status: 'completed',
      createdAt: new Date()
    })
  }
  return messages
}

function testContextTrimmer() {
  console.log('=== 上下文裁剪器测试 ===\n')

  // 测试1：100条消息裁剪
  console.log('测试1：100条消息（每条约1000字符）')
  const messages = generateTestMessages(100)
  const trimmed = trimForChatAPI(messages, 'claude-opus-4')
  console.log(`原始: ${messages.length} 条`)
  console.log(`裁剪后: ${trimmed.length} 条`)
  console.log(`保留最新 ${trimmed.length} 条消息\n`)

  // 测试2：短消息不裁剪
  console.log('测试2：10条短消息')
  const shortMessages = generateTestMessages(10)
  const shortTrimmed = trimForChatAPI(shortMessages, 'claude-opus-4')
  console.log(`原始: ${shortMessages.length} 条`)
  console.log(`裁剪后: ${shortTrimmed.length} 条`)
  console.log('✅ 短消息不应被裁剪\n')

  // 测试3：不同模型配置
  console.log('测试3：不同模型的裁剪差异')
  const testMessages = generateTestMessages(50)
  const claudeTrimmed = trimForChatAPI(testMessages, 'claude-opus-4')
  const gptTrimmed = trimForChatAPI(testMessages, 'gpt-4')
  console.log(`Claude: ${claudeTrimmed.length} 条（限制180k tokens）`)
  console.log(`GPT-4: ${gptTrimmed.length} 条（限制120k tokens）`)
}

testContextTrimmer()
```

### 下午（3小时）：SSE（Server-Sent Events）基础

**学习内容：**
1. SSE协议格式
2. SSE vs WebSocket
3. EventSource API
4. 错误处理和重连

**推荐资源：**
- MDN SSE文档：https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events

**SSE协议格式：**
```
event: message
data: {"type":"chunk","content":"Hello"}

event: message
data: {"type":"chunk","content":" World"}

event: done
data: {"type":"done"}

[空行表示消息结束]
```

**实践任务：创建简单的SSE服务**
```typescript
// app/api/test-sse/route.ts

import { NextRequest } from 'next/server'

export async function GET(request: NextRequest) {
  // 创建ReadableStream
  const stream = new ReadableStream({
    async start(controller) {
      const encoder = new TextEncoder()

      // 发送多个事件
      const messages = ['Hello', ' ', 'World', '!']

      for (const msg of messages) {
        // SSE格式：data: 内容\n\n
        const data = `data: ${JSON.stringify({ content: msg })}\n\n`
        controller.enqueue(encoder.encode(data))

        // 模拟延迟
        await new Promise(resolve => setTimeout(resolve, 500))
      }

      // 发送完成事件
      const done = `data: ${JSON.stringify({ type: 'done' })}\n\n`
      controller.enqueue(encoder.encode(done))

      controller.close()
    }
  })

  // 返回SSE响应
  return new Response(stream, {
    headers: {
      'Content-Type': 'text/event-stream',
      'Cache-Control': 'no-cache',
      'Connection': 'keep-alive'
    }
  })
}
```

**前端消费SSE：**
```typescript
// app/test-sse/page.tsx

'use client'

import { useState } from 'react'

export default function TestSSEPage() {
  const [messages, setMessages] = useState<string[]>([])
  const [isLoading, setIsLoading] = useState(false)

  const handleStart = async () => {
    setIsLoading(true)
    setMessages([])

    try {
      const response = await fetch('/api/test-sse')
      const reader = response.body?.getReader()
      const decoder = new TextDecoder()

      if (!reader) return

      while (true) {
        const { done, value } = await reader.read()
        if (done) break

        // 解码chunk
        const chunk = decoder.decode(value)
        console.log('收到chunk:', chunk)

        // 解析SSE格式
        const lines = chunk.split('\n')
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            const data = JSON.parse(line.slice(6))

            if (data.type === 'done') {
              setIsLoading(false)
            } else if (data.content) {
              setMessages(prev => [...prev, data.content])
            }
          }
        }
      }
    } catch (error) {
      console.error('SSE error:', error)
      setIsLoading(false)
    }
  }

  return (
    <div>
      <h1>SSE测试</h1>
      <button onClick={handleStart} disabled={isLoading}>
        {isLoading ? '接收中...' : '开始接收'}
      </button>
      <div>
        <strong>收到的消息：</strong>
        {messages.join('')}
      </div>
    </div>
  )
}
```

**测试：**
```bash
# 访问测试页面
http://localhost:3007/test-sse
```

**验证标准：**
- [ ] 能解释Token估算的原理
- [ ] 能解释为什么要裁剪上下文
- [ ] 能解释SSE协议格式
- [ ] 能创建简单的SSE服务和客户端

---

### Day 14：聊天API完整实现（6小时）

**重要提示**：这是Week 2的高潮部分！务必理解每一步。

### 上午（3小时）：聊天API路由详解

**实践任务：**
```bash
# 打开聊天API路由（项目最核心的文件）
code app/api/chat/route.ts
```

**详细阅读并添加注释（这里只展示关键步骤）：**
```typescript
// app/api/chat/route.ts 完整流程

import { NextRequest, NextResponse } from 'next/server'
import { auth } from '@/auth'
import { quotaManager } from '@/lib/security/quota-manager'
import { trimForChatAPI } from '@/lib/chat/context-trimmer'
import { createSSETransformStream } from '@/lib/utils/sse-parser'

export async function POST(req: NextRequest) {
  try {
    // ===== 步骤1：认证验证 =====
    const session = await auth()
    if (!session?.user) {
      return NextResponse.json({ error: '未认证' }, { status: 401 })
    }
    const userId = session.user.id

    // ===== 步骤2：解析请求体 =====
    const { messages, conversationId, modelId, settings } = await req.json()

    // ===== 步骤3：配额预留（原子性） =====
    const estimatedTokens = estimateRequestTokens(messages)
    await quotaManager.reserveTokens(userId, estimatedTokens)
    console.log(`[Chat API] 预留配额: ${estimatedTokens} tokens`)

    // ===== 步骤4：上下文裁剪 =====
    const trimmedMessages = trimForChatAPI(messages, modelId)
    console.log(`[Chat API] 裁剪消息: ${messages.length} → ${trimmedMessages.length}`)

    // ===== 步骤5：保存用户消息 =====
    const userMessage = trimmedMessages[trimmedMessages.length - 1]
    await quotaManager.commitTokens(
      userId,
      estimateTokens(userMessage.content),
      0, // 用户消息无预留
      {
        conversationId,
        role: 'user',
        content: userMessage.content,
        userId
      }
    )

    // ===== 步骤6：调用AI API =====
    const aiResponse = await fetch(AI_API_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${API_KEY}`
      },
      body: JSON.stringify({
        model: modelId,
        messages: trimmedMessages.map(m => ({
          role: m.role,
          content: m.content
        })),
        stream: true
      })
    })

    if (!aiResponse.ok) {
      // 失败：释放预留配额
      await quotaManager.releaseTokens(userId, estimatedTokens)
      throw new Error('AI API调用失败')
    }

    // ===== 步骤7：流式响应处理 =====
    let fullContent = ''
    let fullReasoning = ''

    const transformStream = createSSETransformStream({
      // 流式接收chunk
      onChunk: (delta: string, reasoning?: string) => {
        fullContent += delta
        if (reasoning) fullReasoning += reasoning
      },

      // 完成时保存助手消息
      onComplete: async () => {
        const actualTokens = estimateTokens(fullContent)
        const adjustment = actualTokens - estimatedTokens

        console.log(`[Chat API] 实际使用: ${actualTokens} tokens`)
        console.log(`[Chat API] 调整配额: ${adjustment} tokens`)

        // 保存助手消息并调整配额
        await quotaManager.commitTokens(
          userId,
          actualTokens,
          estimatedTokens,
          {
            conversationId,
            role: 'assistant',
            content: fullContent,
            userId,
            metadata: fullReasoning ? { reasoning: fullReasoning } : undefined
          }
        )
      },

      // 错误时回滚
      onError: async (error) => {
        console.error('[Chat API] 错误:', error)
        await quotaManager.releaseTokens(userId, estimatedTokens)
      }
    })

    // ===== 步骤8：返回流式响应 =====
    return new Response(
      aiResponse.body!.pipeThrough(transformStream),
      {
        headers: {
          'Content-Type': 'text/event-stream',
          'Cache-Control': 'no-cache',
          'Connection': 'keep-alive'
        }
      }
    )

  } catch (error) {
    console.error('[Chat API] 异常:', error)
    return NextResponse.json(
      { error: error.message },
      { status: 500 }
    )
  }
}

// 辅助函数：估算请求token
function estimateRequestTokens(messages: ChatMessage[]): number {
  return messages.reduce((total, msg) => {
    return total + estimateTokens(msg.content, 2)
  }, 0)
}
```

**画出完整流程图：**
```
POST /api/chat
  ↓
1. 认证验证 (auth())
  ├─ 成功 → 获取userId
  └─ 失败 → 401错误
  ↓
2. 解析请求体
  ├─ messages（消息历史）
  ├─ conversationId（对话ID）
  ├─ modelId（模型ID）
  └─ settings（设置）
  ↓
3. 配额预留（原子性）
  ├─ estimateRequestTokens() 估算tokens
  ├─ quotaManager.reserveTokens()
  ├─ 成功 → 继续
  └─ 失败 → 429错误（配额不足）
  ↓
4. 上下文裁剪
  ├─ trimForChatAPI()
  └─ 保留最新N条消息
  ↓
5. 保存用户消息
  ├─ quotaManager.commitTokens()
  └─ 创建Message记录
  ↓
6. 调用AI API
  ├─ fetch(AI_API_URL, { stream: true })
  ├─ 成功 → 获取stream
  └─ 失败 → releaseTokens() + 错误响应
  ↓
7. 流式响应处理
  ├─ createSSETransformStream()
  ├─ onChunk: 累积content
  ├─ onComplete: 保存助手消息 + 调整配额
  └─ onError: releaseTokens()
  ↓
8. 返回SSE流
  └─ Response(stream, { headers: SSE headers })
```

### 下午（3小时）：测试聊天API

**实践任务：编写完整测试**
```typescript
// scripts/test-chat-api.ts

async function testChatAPI() {
  console.log('=== 聊天API完整测试 ===\n')

  // 步骤1：登录获取session
  console.log('步骤1：登录...')
  const loginRes = await fetch('http://localhost:3007/api/auth/signin', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      email: 'test@test.com',
      code: process.env.DEV_LOGIN_CODE
    })
  })
  const cookies = loginRes.headers.get('set-cookie')
  console.log('登录成功')

  // 步骤2：发送聊天请求
  console.log('\n步骤2：发送聊天请求...')
  const chatRes = await fetch('http://localhost:3007/api/chat', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Cookie': cookies || ''
    },
    body: JSON.stringify({
      messages: [
        { role: 'user', content: 'Hello, how are you?' }
      ],
      conversationId: 'test-conversation',
      modelId: 'claude-3-5-haiku-20241022'
    })
  })

  console.log('响应状态:', chatRes.status)
  console.log('响应头:', Object.fromEntries(chatRes.headers))

  // 步骤3：读取流式响应
  console.log('\n步骤3：接收流式响应...')
  const reader = chatRes.body?.getReader()
  const decoder = new TextDecoder()

  let fullResponse = ''

  if (reader) {
    while (true) {
      const { done, value } = await reader.read()
      if (done) break

      const chunk = decoder.decode(value)
      console.log('收到chunk:', chunk)

      // 解析SSE格式
      const lines = chunk.split('\n')
      for (const line of lines) {
        if (line.startsWith('data: ')) {
          const data = JSON.parse(line.slice(6))
          if (data.content) {
            fullResponse += data.content
            process.stdout.write(data.content) // 实时显示
          }
        }
      }
    }
  }

  console.log('\n\n步骤4：完整响应:')
  console.log(fullResponse)

  console.log('\n✅ 测试完成')
}

testChatAPI().catch(console.error)
```

**运行测试：**
```bash
npx tsx scripts/test-chat-api.ts
```

**验证标准：**
- [ ] 能解释聊天API的8个关键步骤
- [ ] 能解释预留-提交-回滚的配额管理模式
- [ ] 能解释流式响应的处理流程
- [ ] 能独立运行并理解测试脚本的输出

**Week 2 总结：**
- [ ] 深入理解Prisma和数据库设计
- [ ] 掌握NextAuth认证系统
- [ ] 完全理解聊天API的完整链路
- [ ] 理解原子性操作和并发控制

---

由于篇幅限制，我将继续创建Week 3-5的学习计划。要我继续吗？
